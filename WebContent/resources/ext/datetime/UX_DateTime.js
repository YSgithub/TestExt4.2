...
//            '<td>{%this.renderSecondBtn(values,out)%}</td>',//秒数按钮控件
            '</tr></tbody>',
            '</table>',
...
//            me.secondBtn.finishRender();//秒数按键事件绑定
...
//                date.setMinutes(me.secondBtn.getValue());
                date.setSeconds("00");//直接设置默认值为00
            }
            me.value=date;
            me.update(me.value);
            return me;
...
//        console.log(date.dayNames);
        proto.dayNames =  ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"];
